...
Routix is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[Routix Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.routix/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.routix/usage.json`, `~/.routix/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.routix/db.json`)
- usage stats: `~/.routix/usage.json`
- request log lines: `~/.routix/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[Routix Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.routix` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/routix && npm run build`
- Build Docker image: `cd /root/dev/routix && docker build -t routix .`
