...
const hasRoutixConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["routix"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("routix");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasRoutix: hasRoutixConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "Routix", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "Routix settings removed from Kilo Code" });
  } catch (error) {
