...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-routix.env");
};
...
const hasRoutixConfig = (config) => {
  ...
  if (providers["routix"]) return true;
...
export async function GET() {
  ...
  const hasRoutix = hasRoutixConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_ROUTIX_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile routix",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["routix"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_ROUTIX_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "routix configuration removed from jcode",
    });
  } catch (error) {
