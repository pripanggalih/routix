...
const hasRoutixConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "Routix");
};

const getRoutixEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "Routix") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getRoutixEntry(config);
...
// POST - Apply Routix config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing Routix entry or append
    const idx = config.findIndex((e) => e.name === "Routix");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove Routix entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "Routix");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "Routix removed from Copilot config",
    });
  } catch (error) {
