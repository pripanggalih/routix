...
// Check if settings has Routix config
const hasRoutixConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["routix"];
};

// Read per-agent models.json and return current model id (without "routix/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["routix"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasRoutix: hasRoutixConfig(settings),
...
// POST - Update Routix settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `routix/${model}`;

    // Remove all old routix/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("routix/"))
    ...
    // Add fresh routix models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`routix/${m}`] = {};
    });

    // Remove old routix model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("routix/")) {
    ...
    // Update models.providers.routix with all models
    settings.models.providers["routix"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `routix/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Routix settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Routix from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["routix"];
    ...
    // Remove routix models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("routix/"));
    ...
    // Reset agents.defaults.model.primary if it uses routix
    if (settings.agents?.defaults?.model?.primary?.startsWith("routix/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "Routix settings removed successfully",
    });
  } catch (error) {
