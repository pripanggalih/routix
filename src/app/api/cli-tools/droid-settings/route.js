...
// Check if settings has Routix customModels
const hasRoutixConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Routix"));
};
...
// POST - Update Routix customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Routix configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Routix"));
...
// DELETE - Remove Routix customModels only (keep other settings)
