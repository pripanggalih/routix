  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "routix")
        : path.join(os.homedir(), ".routix"),
      "mitm",
      ".mitm.pid"
    );
...
// Collect PIDs of all routix-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from routix install dir or wrapper cli.js
        const isAppProcess = lower.includes("routix") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("routix") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "routix");
  }
  return path.join(os.homedir(), ".routix");
}
...
// Resolve npx/routix binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx routix` — works regardless of global bin path changes after npm i -g
