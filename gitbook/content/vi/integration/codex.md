...
Tích hợp Routix với OpenAI Codex CLI để định tuyến request API OpenAI qua hệ thống routing thông minh của Routix.
...
Routix cung cấp các model Codex sau:
...
1. Xác minh API key đúng trong Routix dashboard
...
1. Xác minh tên model khớp với cấu hình Routix
2. Kiểm tra kết nối provider OpenAI đang hoạt động trong Routix dashboard
...
Đảm bảo bạn đã cấu hình API key trong Routix cloud dashboard.
