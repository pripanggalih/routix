...
Integrate Routix with Cline VSCode extension to route your AI requests through Routix's intelligent routing system.

## Prerequisites

- Visual Studio Code installed
- Cline extension installed from VSCode marketplace
- Routix running locally or cloud endpoint configured
...
Set the base URL to your Routix endpoint:
...
**Steps:**
1. In the **Base URL** field, enter your Routix endpoint
...
3. The key should start with `sk-routix-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your Routix configuration
...
4. Cline will use Routix to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using Routix
...
3. Get AI-powered explanations through Routix

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use Routix to understand context and make changes
...
1. Verify Routix is running: `curl http://localhost:20128/health`
...
2. Verify your Routix instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check Routix logs for any errors

## Advanced Configuration

### Using Cloud Endpoint

To use Routix cloud endpoint instead of localhost:

1. In Cline settings, set Base URL to: `https://routix.web.id`
2. Make sure you have configured your API key in the Routix cloud dashboard
...
## Integration with Routix Features

### Model Routing

Routix automatically routes your requests to the best available provider based on:
- Model availability
...
If a provider fails, Routix automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through Routix dashboard:
- Total requests
- Token usage
- Cost per model
