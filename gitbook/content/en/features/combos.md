...
Combos are **custom fallback chains** that you create in the dashboard. Instead of using a single model, you define a sequence of models that Routix tries in order.
...
Routix automatically tries each model in sequence until one succeeds.
...
When limit reached, Routix skips paid models and uses free tier only.
