...
Despliega Routix en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.routix` | Ruta de almacenamiento de la base de datos |
...
```bash
sudo mkdir -p /var/lib/routix
sudo chown $USER:$USER /var/lib/routix
```
...
Crea `/etc/nginx/sites-available/routix`:
...
# Si NO usas proxy reverso, permite los puertos de Routix
...
tar -czf routix-backup-$(date +%Y%m%d).tar.gz /var/lib/routix

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/routix-$(date +\%Y\%m\%d).tar.gz /var/lib/routix
```
...
```bash
# Verificar si Routix está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/routix
chmod 755 /var/lib/routix
```
